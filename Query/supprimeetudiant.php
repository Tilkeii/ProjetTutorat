...
if(isset($_POST["iduser"])){
    ...
    foreach($resultat as $r){
        ...
        $req->execute(array(
                ...
                ) or die(print_r('9'.$bdd->errorInfo(), true));
    ...
    $reqBis->execute(array('iduser1' => $_POST["iduser"])) or die(print_r('6'.$bdd->errorinfo(),true));
    ...
    //On recupères les etudiants qui voulaient aider l'élève qui va être supprimer
    ...
    $reqBis->execute(array('iduser' => $_post["iduser"])) or die(print_r('7'.$bdd->errorinfo(),true));
    $resultat = $reqBis->fetchAll();
    ...
    foreach($resultat as $r){
        ...
        $req->execute(array(
	    			'iduser' => $r['numero_etudiant'])
                ) or die(print_r('8'.$bdd->errorInfo(), true));
        $resultatfind = $req->fetch();
        // Envoi du mail
        $mail_demandeur = new Helper_Mail();
        $mail_demandeur
